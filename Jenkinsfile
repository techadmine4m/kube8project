pipeline {
  environment {
    registry = "gcr.io/smiling-destiny-184908/kube8project"
  ...
  stages {
    stage('Cloning Git') {
      steps {
        git url: 'https://github.com/techadmine4m/kube8project', branch: 'master',
    ...
    stage('Deploy Image') {
      ...
      stage('Deploy to GKE') {
            steps{
                sh "sed -i 's/kube8project:latest/kube8project:$BUILD_NUMBER/' sample-web-deployment.yaml"
